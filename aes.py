...
# hàm sub_bytes, thay thế bytes với bảng s_box
def sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = s_box[s[i][j]]
...
# hàm sub_bytes, thay thế bytes với bản s_box nghịch
def inv_sub_bytes(s):
    for i in range(4):
        for j in range(4):
...
#     matrix = []
#
#     # Iterate over the text array in chunks of 4 bytes
#     for i in range(0, len(text), 4):
#         # Get a chunk of 4 bytes from the text array
#         chunk = text[i:i + 4]
#
#         # Convert the chunk into a list
#         row = list(chunk)
#
